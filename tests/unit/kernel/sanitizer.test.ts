...
describe('Sanitizer', () => {
  it('should have 63 patterns across 27 categories', () => {
  ...
  it('should detect direct override injection', () => {
    ...
    expect(result.threats[0].category).toBe('direct_override');
  });
  ...
  it('should detect command injection', () => {
    ...
    expect(result.safe).toBe(false);
    expect(result.threats.some(t => t.category === 'command')).toBe(true);
  });
  ...
  it('should detect path traversal', () => {
    ...
    expect(result.safe).toBe(false);
    expect(result.threats.some(t => t.category === 'path')).toBe(true);
  });

  it('should detect null byte injection', () => {
    const result = sanitize('file.txt%00.jpg', 'untrusted');
    expect(result.safe).toBe(false);
    expect(result.threats.some(t => t.category === 'null_byte')).toBe(true);
  });

  it('should detect XML injection', () => {
    const result = sanitize('<![CDATA[malicious]]>', 'untrusted');
    expect(result.safe).toBe(false);
    expect(result.threats.some(t => t.category === 'xml')).toBe(true);
  });
